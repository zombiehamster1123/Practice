// IPCDemo.cpp : Defines the entry point for the console application.
//
...
template <typename T>
bool Problem(T data, const char* name)
{
	if (!data)
	{
		cout << "Cannot create " << name << "!" << endl 
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	bool bSuccess = CreateProcess(TEXT("..\\Debug\\IPCWoker.exe"),
		TEXT("red"), NULL, NULL, FALSE, 0, NULL, NULL, &startupInfoRed,
		&processInformationRed);
	if (Problem(bSuccess, "process red"))
		return 1;
	...
	if (Problem(bSuccess, "process blue"))
		return 1;
