...
private:
	// Mutex: allows threads to have access the same
	// resource but only one at a time... need
	// permission first before it can have access
...
public:
	...
	HANDLE Create(LPVOID lpParamter,
		DWORD dwInitialize = STATE_ASYNC, DWORD dwCreationFlag = 0);
